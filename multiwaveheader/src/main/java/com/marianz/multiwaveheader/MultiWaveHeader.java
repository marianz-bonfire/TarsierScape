...
public class MultiWaveHeader extends ViewGroup {
    ...
    public MultiWaveHeader(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mWaveAmplitude =10;// ta.getInt(R.styleable.MultiWaveHeader_mwhWaveAmplitude, Util.dp2px(1));
...
//    }
    ...
    private void updateLinearGradient(int width, int height) {
        int startColor = mStartColor;// ColorUtils.setAlphaComponent(mStartColor, (int)(mColorAlpha*255));
    ...
    protected void updateWavePath(int w, int h) {
        for (Wave wave : mltWave) {
            wave.updateWavePath(w, h, (mWaveHeight / 2)-mWaveAmplitude, mEnableFullScreen, mCurProgress);
        }
    }
    ...
    protected void updateProgress(float progress) {
        ...
        if (mEnableFullScreen) {
            for (Wave wave : mltWave) {
                wave.updateWavePath(thisView.getWidth(), thisView.getHeight()-mWaveAmplitude, mCurProgress);
            }
        }
        if (!mIsRunning) {
            invalidate();
    ...
    public void setWaveHeight(int waveHeight) {
        ...
        if (!mltWave.isEmpty()) {
            View thisView = this;
            updateWavePath(thisView.getWidth(), thisView.getHeight()-mWaveAmplitude);
        }
    }
